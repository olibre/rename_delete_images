...
processMD5output()
{
  ...
  do
      case $line in
      \\*)   echo "${line:1:33}"$'\t0\t'"${line:35}"    ;;
        *)   echo "${line:0:32}"$'\t1\t'"${line:34}"    ;;
    esac
  done
}
...
while read -r LINE[i++]; do :; done < <(
  ...
  sort -zu --parallel=4 |              #skip duplicated path/filenames
  xargs -0 --max-procs=8 du -0Lb |      #retrieve size in bytes
  grep -vzZ '^0' |                      #ignore empty files
  sort -zu --parallel=4 |     #sort by size
  #keep files having same size
  awk 'function printBut1st() { for(i=2;i<=NF;i++) printf "%s",$i (i==NF?ORS:OFS) }
       {  if(s==$1) { printBut1st(); if (l!="") {$0=l; l=""; printBut1st()} }
         #if(s==$1) { print;         if (l!="") { print l; l="" } }
          else      { s=$1; l=$0 } }' RS='\0' ORS='\0' FS='\t' OFS='\t' |
